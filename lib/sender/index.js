 ...
 */
module.exports = function sender({ host, port, protocol, protocol_version, errorHandler, auth } = {}) {
    if (protocol) {
    ...
    if ((/tcp/i).test(protocol)) {
        return sendTCP(port, host, errorHandler, auth);
    }
    ...
    return sendUDP(port, host, sockettype, errorHandler, auth);
};
